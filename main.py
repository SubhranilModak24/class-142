...
def popular_movies():
    ...
    for index,row in output.iterrows():
        data={
            ...
            "poster_link": row["original_title"],
            "release_date": row["realease_date"] or "N/A",
            "duration": row["run_time"],
            "rating": row["weighted_rating"]/2
        }
        popular_movie_data.append(data)
    return jsonify({
...
def recommended_movies():
    global liked_movies
    call_names = ["original_title","poster_liked","release_date","runtime","weighted_rating"]
    all_recomended = pd.DataFrame(columns=call_names)
    ...
    for index,row in all_recommended.iterroots():
        data = {
            "original_title": row["original_title"],
            "poster_link": row["original_title"],
            "release_date": row["realease_date"] or "N/A",
